#!/usr/bin/env python3
"""Bump the TELEPORT_VERSION and commit the change.

Usage (run from anywhere; operates on the repo root):

    python scripts/bump_version.py              # bump patch (default)
    python scripts/bump_version.py --patch      # 0.1.12.0 -> 0.1.13.0
    python scripts/bump_version.py --minor      # 0.1.12.0 -> 0.2.0.0
    python scripts/bump_version.py --major      # 0.1.12.0 -> 1.0.0.0
    python scripts/bump_version.py --hotfix     # 0.1.12.0 -> 0.1.12.1

Only one of --major/--minor/--patch/--hotfix may be given. Bumping a segment
zeros every segment after it. Must be run on `main`; the new TELEPORT_VERSION
is committed.
"""
...
_PARTS = ("major", "minor", "patch", "hotfix")


def bump(version: str, part: str) -> str:
    """Return `version` with `part` incremented and lower segments zeroed.

    Raises ValueError on an unknown part or a malformed version.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    group.add_argument("--patch", action="store_const", const="patch", dest="part",
                       help="bump PATCH, zero HOTFIX (default)")
    group.add_argument("--hotfix", action="store_const", const="hotfix", dest="part",
                       help="bump HOTFIX")
