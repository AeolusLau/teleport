...
- **背景**:`gn gen` 对 release 配置(`teleport_deployment_env="release"`,`src/gn/args/release.mac.gn`)会命中 `src/teleport.gni` 一条**故意设计**的 fail-closed 断言:该环境要求 `teleport_release_policy_key_is_real=true`,而后者默认 `false`——生产 KMS 根密钥按设计从不入库,`kReleasePolicyKey` 目前仍是占位值。**2026-08-10 更新**:release 的**恢复根**已由离线仪式产出并烤入(`a45361da…`),故本条现在只卡在**主根**一把上;`gen_policy_verification_key.py --check --require-real --env release` 的失败原因已收敛为一条。这条断言随纳管强制锁 Layer 1 特性(commit `1adc884`,2026-06-22)一起落地。
