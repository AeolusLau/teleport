...
- 上游基线钉死在 `CHROMIUM_VERSION`(当前 **152.0.7977.65**);升级流程见 `docs/chromium-upgrade-runbook.md`。
...
cd "$TELEPORT_CHROMIUM_ROOT"/<release_branch>/src   # <release_branch> = CHROMIUM_VERSION 前三段,如 152.0.7977;设了 $TELEPORT_CHROMIUM_DIR 则直接 cd 到它
...
Set-Location $env:TELEPORT_CHROMIUM_ROOT\152.0.7977\src
...
- **桌面 Mac/Win 同线、Linux 取子集**:实测 M151 已发布序列 Mac 与 Win64 完全一致(`.76 .75 .72 .71 .47 .34`),Linux 是同一序列的子集(`.75 .71`)——桌面三平台共用同一条发布线,故**单一 `CHROMIUM_VERSION` pin 服务全部桌面平台是正确的**,不需要按平台分别维护;Linux 落后不构成问题(同一分支源码,只是比 Google 推给 Linux 用户的多带若干还没轮到 Linux 的修复)。`check_upstream_release.py` 若报 mac/win64 版本不一致会显式告警,届时需人工判断。**M152 复核结论一致**:mac 与 win64 的 M152 已发布序列同为 `.65 .64 .54 .42`,Linux stable 停在 `.64`(仍是子集),故本次 pin 取两者一致的最后一个已发布版本 `152.0.7977.65`。
...
- **源码 symlink**:`src/` 经符号链接挂进 `chromium/src/teleport`,M148、M151、M152 上 GN + clang 均已验证可正常解析与编译(无需退路)。
- **上游注入点会随里程碑漂移,每次升级需重新核对**:M148 上 `//teleport` 的 sources/deps 挂在 `chrome/browser/BUILD.gn` 的 `static_library("browser")`;M151 把该 target 拆分,**改挂 `source_set("core")`**(`static_library("browser")` 已无自有 sources,只在文件里剩一段解释 GN 循环依赖机制的注释,提到它作为历史示例,不代表真实构造)。**M152 复核:未再漂移**,仍挂在 `source_set("core")`(`chrome/browser/BUILD.gn:907`)。启动 banner 调用一直未受影响,仍在 `chrome/browser/chrome_browser_main.cc` 的 `PreMainMessageLoopRun`。下次升级前仍要按这个模式(`grep '"//teleport"' chrome/browser/BUILD.gn` 找当前挂载的 target 名)重新核对——「上次没变」不构成下次不变的理由。
